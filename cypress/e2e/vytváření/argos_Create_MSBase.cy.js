describe("Vytvoření výzkumného záměru", () => {
  it("vytvoření záměru", () => {
    ...
    cy.get('input[aria-label="Název *"]').type("MS Base");
    cy.get('textarea[aria-label="Popis *"]').type("Automatický test pro MS Base");
    cy.get('input[aria-label="Rozsah dat *"]').click();
    cy.get('.q-virtual-scroll__content').contains('.q-item__label', 'MSBase').click();
