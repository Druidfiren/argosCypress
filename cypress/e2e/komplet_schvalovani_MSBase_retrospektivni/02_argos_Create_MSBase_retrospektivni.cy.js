describe("Vytvoření výzkumného záměru", () => {
  it("vytvoření záměru", () => {
    ...
    cy.get('input[aria-label="Název *"]').type("MSBase retrospektivní");
    cy.get('textarea[aria-label="Popis *"]').type("Automatický test pro MSBase retrospektivní");
    cy.get('input[aria-label="Rozsah dat *"]').click();
    cy.get('.q-menu .q-item').contains('MSBase - retrospektivní').click();
